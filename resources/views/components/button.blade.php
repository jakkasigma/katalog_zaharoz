...
<button type="{{ $type }}" {{ $attributes->merge(['class' => "inline-flex items-center justify-center px-5 py-3 font-mono text-sm font-semibold uppercase tracking-[0.18em] transition focus:outline-none focus:ring-4 focus:ring-lens/30 disabled:cursor-not-allowed disabled:opacity-60 {$classes}"]) }}>
    {{ $slot }}
</button>
